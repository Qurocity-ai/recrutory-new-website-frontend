...
const CardDetails = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`https://recrutory-new-website-backend-3.onrender.com/user/${id}`)
      .then((response) => {
  ...
  }, [id]); // Removed `user` from dependency array
          ...
          <div className={styles.detailsCard}>
            <div className={styles.top}>
              <h2 className={styles.name}>
                {user?.name} | <span>Total work experience: {user?.total_experience || 0} Years</span>
              </h2>
            </div>
            <div className={styles.cDetails}>
              <p className={styles.proficiency}>
                <strong>Proficiency:</strong> {frenchProficiency.level || "N/A"} | {frenchProficiency.certification || "N/A"}
              </p>
