...
function NewPage() {
  ...
  const sliderSettings = {
    dots: false, // Hide dots and use a clickable line
    infinite: true,
    speed: 500,
    slidesToShow: 1,
    slidesToScroll: 1,
  ...
  if (isMobile) {
    return (
      <div className={styles.mobileWrapper}>
        <Slider {...sliderSettings} className={styles.mobileCarousel} ref={sliderRef}>
          {slides.map(({ image }, index) => (
            <div key={index}>
              <img src={image} alt={`Slide ${index + 1}`} className={styles.slideImage} />
            </div>
        ...
        <div className={styles.lineNavigation}>
          {slides.map((_, index) => (
            <div
              key={index}
              className={`${styles.lineSegment} ${index === activeIndex ? styles.activeSegment : ''}`}
